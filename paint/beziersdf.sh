...
float upper_bound_lagrange5(float a0, float a1, float a2, float a3, float a4)
{
	...
	vec4 neg1 = max(-coeffs1,vec4(0.0, 0.0, 0.0, 0.0));
	float neg2 = max(-a4,0.);
...
float lower_bound_lagrange5(float a0, float a1, float a2, float a3, float a4){
	...
	vec4 neg1 = max(-coeffs1,vec4(0, 0, 0, 0));
	float neg2 = max(-a4,0.);
...
vec3 parametric_cub_bezier(float t, vec3 p0, vec3 p1, vec3 p2, vec3 p3){
	vec3 a0 = (-p0 + 3. * p1 - 3. * p2 + p3);
	vec3 a1 = (3. * p0  -6. * p1 + 3. * p2);
	vec3 a2 = (-3. * p0 + 3. * p1);
	vec3 a3 = p0;
...
int solve_quartic(vec4 coeffs, inout vec4 s){
    ...
    //}
    {
		...
		cubic_coeffs.x = 1.0/2. * r * p - 1.0/8. * q * q;
		cubic_coeffs.y = - r;
		cubic_coeffs.z = - 1.0/2. * p;
		vec3 ss;
		solve_cubic(cubic_coeffs, ss);
		s.xyz = ss;
		...
		vec2 sss;
		num = solve_quadric(quad_coeffs, sss);
		s.xy = sss;
		quad_coeffs[0]= z + u;
		quad_coeffs[1] = q < 0. ? v : -v;
...
float cubic_bezier_sign(vec2 uv, vec2 p0, vec2 p1, vec2 p2, vec2 p3){
	...
	vec3 roots = vec3(1e38, 1e38, 1e38);
...
float cubic_bezier_dis(vec3 uv, vec3 p0, vec3 p1, vec3 p2, vec3 p3, inout float T){

	//switch points when near to end point to minimize numerical error
	//only needed when control point(s) very far away
	#if 0
	vec3 mid_curve = parametric_cub_bezier(.5,p0,p1,p2,p3);
	vec3 mid_points = (p0 + p3)/2.;

	vec3 tang = mid_curve-mid_points;
	vec3 nor = vec3(tang.y,-tang.x);
	...
	vec4 roots_drv = vec4(10, 11, 12, 13);

	int num_roots_drv = solve_quartic(c1,roots_drv);
	if(roots_drv.x > roots_drv.y)
	{ 
		float tmp = roots_drv.x;
		roots_drv.x = roots_drv.y;
		roots_drv.y = tmp; 
	}
	...
	vec3 a = vec3(1e38, 1e38, 1e38);
	vec3 b = vec3(1e38, 1e38, 1e38);
	...
	if(num_roots_drv==4){
		...
		if(num_roots_drv==2){
			...
			vec3 roots_snd_drv=vec3(1e38, 1e38, 1e38);
			int num_roots_snd_drv=solve_cubic(c2,roots_snd_drv);

			vec2 roots_trd_drv=vec2(1e38, 1e38);
	...
	for(int i=0;i<3;i++){
		if(i < num_roots){
			...
			vec3 to_curve = uv - parametric_cub_bezier(roots[i],p0,p1,p2,p3);
            float thisD = dot(to_curve, to_curve);
            if(thisD < d0 && roots[i] >= 0.0 && roots[i] < 1.0)
            {
                d0 = thisD;
                T = roots[i];
            }
...
float sideDot(vec3 p, vec3 p0, vec3 p1, vec3 p2, vec3 p3, vec3 z, float tNearest)
{
    vec3 s1 = parametric_cub_bezier(tNearest - 0.01, p0, p1, p2, p3);
    vec3 s2 = parametric_cub_bezier(tNearest + 0.01, p0, p1, p2, p3);
    vec3 s1p = normalize(s1-p);
    vec3 s2p = normalize(s2-p);
    return dot(cross(s1p, s2p), z);
    

}

float segmentDistance(vec3 p1, vec3 p2, vec3 p)
{
    vec3 p12 = p2-p1;
    float l = length(p12);
    vec3 p12N = normalize(p12);
    vec3 pp = p - p1;
    float u = dot(pp, p12N);
    float v = sqrt(max(dot(pp, pp)- u * u, 0.000001));
    if(u > l || u < 0.0)
    { return 10000.0; }
    return v;
