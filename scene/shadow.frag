...
END_PARAMS
{
...
#else
	float normalShift = uNormalOffset.x;
#endif
	...
	texPos.xyz /= texPos.w;
	vec4 offsetTexPos = mulPoint( uShadowTextureMatrix, offsetWorldPos );
	offsetTexPos.xyz /= offsetTexPos.w;
	
	vec4 normalTap = texture2DLod(tNormal, screenCoord.xy, 0.0);
	...
	vec3 offsetObjPos = mulPoint( uShadowMatrix, offsetWorldPos ).xyz;
	...
	transScale *= 0.2;						//divide by 5 to match the exponential curve to world units by appearance
	...
	#ifdef SHADOWS
	#ifdef AREA_SHADOWS
		// Area shadows are based on PCSS (Percentage Closer Soft Shadows)
		// https://developer.download.nvidia.com/shaderlibrary/docs/shadow_PCSS.pdf
		// It is modified somewhat to match against raytracing and how our scene unit works
		...
		//find our search space size
		#ifdef SPOT_SHADOWS
			vec2 searchSpaceSize = lightSize / offsetObjDepth;
		#else
			...
			vec2 searchSpaceSize = offsetObjDepth * lightSize * uAreaScale * 0.1;
		#endif
		...
		if( occluderCount > 0.0 )
		{
			...
			#ifdef SPOT_SHADOWS	
				vec2 pw = ((offsetObjDepth - avgOccluder) * lightSize) / (avgOccluder);
				vec2 pwt = pw / offsetObjDepth;
			#else
				//DIRECTIONAL
				//light size at distance to avg occluder, divided by projection area. decoupled from searchSpaceSize
				vec2 pwt = (offsetObjDepth-avgOccluder) * lightSize * uAreaScale * 0.1;
			#endif
	...
	#ifdef SPOT_SHADOWS
		//spotlight vignette
		vec2 vignette = uVignette.x*offsetTexPos.xy - uVignette.yy;
		vignette.x = saturate( uVignette.z - uVignette.z*dot( vignette, vignette ) );
		shadow *= vignette.x;
	#endif
